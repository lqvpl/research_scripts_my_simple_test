package spark.core
{
   ...
   public class SpriteVisualElement extends FlexSprite implements IVisualElement, IMXMLObject, IFlexModule
   {
      ...
      private function setActualSize(param1:Number, param2:Number) : void
      {
         ...
         if(this._width != param1 && this._layoutFeatures)
         {
         ...
         {
            _loc3_ = !!isNaN(this._viewWidth)?Number(this.naturalWidth):Number(this._viewWidth);
            _loc4_ = !!isNaN(this._viewHeight)?Number(this.naturalHeight):Number(this._viewHeight);
            _loc5_ = _loc3_ != 0?Number(this._width / _loc3_):Number(1);
            _loc6_ = _loc4_ != 0?Number(this._height / _loc4_):Number(1);
            if(_loc5_ != 1 || _loc6_ != 1 || this._layoutFeatures)
            {
               if(this._layoutFeatures == null)
               {
      ...
      protected function invalidateSize() : void
      {
         ...
         {
            _loc3_ = this.parent;
            while(this.nestedSpriteVisualElement)
            {
      ...
      public function get minHeight() : Number
      {
         ...
         return this.resizeMode == ResizeMode.SCALE?Number(0):Number(this.preferredHeight);
      }
      ...
      private function setModuleFactoryInChildrenOf(param1:DisplayObjectContainer, param2:IFlexModuleFactory) : void
      {
         ...
         while(_loc4_ < _loc3_)
         {
            ...
            if(_loc5_ is IFlexModule)
            {
               _loc6_ = IFlexModule(_loc5_);
               if(_loc6_.moduleFactory == null || _loc6_.moduleFactory == this._moduleFactory)
               {
      ...
      public function get minWidth() : Number
      {
         ...
         return this.resizeMode == ResizeMode.SCALE?Number(0):Number(this.preferredWidth);
      }
      ...
      {
         var _loc4_:Matrix = !!param3?this.nonDeltaLayoutMatrix():null;
         if(!_loc4_)
         {
            return this.x;
         }
         var _loc5_:Point = new Point(0,0);
         MatrixUtil.transformBounds(this.preferredWidth,this.preferredHeight,_loc4_,_loc5_);
         return _loc5_.x;
      }
      ...
      public function setLayoutBoundsSize(param1:Number, param2:Number, param3:Boolean = true) : void
      {
         ...
         var _loc5_:Point = MatrixUtil.fitBounds(param1,param2,_loc4_,this._explicitWidth,this._explicitHeight,this.preferredWidth,this.preferredHeight,this.minWidth,this.minHeight,this.maxWidth,this.maxHeight);
         if(!_loc5_)
         {
            this.setActualSize(this.preferredWidth,this.preferredHeight);
         }
         else
         {
            this.setActualSize(_loc5_.x,_loc5_.y);
         }
      }
      ...
      protected function transformWidthForLayout(param1:Number, param2:Number, param3:Boolean = true) : Number
      {
         ...
         if(param3)
         {
            _loc4_ = this.nonDeltaLayoutMatrix();
            if(_loc4_)
            {
      ...
      private function applyMask() : void
      {
         ...
         if(this.maskTypeChanged)
         {
            ...
            if(this._mask)
            {
               MaskUtil.applyMaskType(this._mask,this._maskType,this.luminosityInvert,this.luminosityClip,this);
            }
         }
      }
   }
