package spark.components.supportClasses
{
   ...
   public class TouchScrollHelper
   {
      ...
      mx_internal function startScrollWatch(param1:Event, param2:Boolean = true, param3:Boolean = true, param4:Number = 0, param5:Number = NaN) : void
      {
         ...
         this.startTime = GetTimerUtil.getTimer();
         ...
         if(param1 is MouseEvent && param1.type == MouseEvent.MOUSE_DOWN)
         {
            ...
            this.addMouseEventHistory(_loc6_.stageX,_loc6_.stageY,GetTimerUtil.getTimer());
         }
      ...
      private function addMouseEventHistory(param1:Number, param2:Number, param3:Number) : Point
      {
         ...
         this.mouseEventTimeHistory[_loc7_] = !!this.isIOS?int(param3 - this.startTime):int(GetTimerUtil.getTimer() - this.startTime);
      ...
      private function installMouseListeners() : void
      {
         ...
         _loc1_ = this.target.systemManager;
         ...
         if(_loc1_.stage && _loc1_.isTopLevelRoot())
         {
         ...
         this.target.systemManager.deployMouseShields(true);
      ...
      private function uninstallMouseListeners() : void
      {
         ...
         this.target.systemManager.deployMouseShields(false);
      ...
      private function sbRoot_mouseMoveHandler(param1:MouseEvent) : void
      {
         ...
         var _loc4_:TouchInteractionEvent = null;
         ...
         if(!this.isScrolling)
         {
            ...
            if(_loc3_)
            {
               _loc4_ = new TouchInteractionEvent(TouchInteractionEvent.TOUCH_INTERACTION_STARTING,true,true);
               _loc4_.relatedObject = this.target;
               _loc4_.reason = TouchInteractionReason.SCROLL;
               _loc5_ = this.dispatchBubblingEventOnMouseDownedDisplayObject(_loc4_);
               if(!_loc5_)
               {
               ...
               _loc6_ = new TouchInteractionEvent(TouchInteractionEvent.TOUCH_INTERACTION_START,true,true);
               _loc6_.relatedObject = this.target;
               ...
               {
                  _loc7_ = Math.max(Math.abs(_loc2_.x),Math.abs(_loc2_.y));
                  if(_loc7_ >= this.scrollSlop)
                  {
         ...
         if(this.isScrolling)
         {
            ...
            if(!this.dragTimer)
            {
               _loc14_ = !!isNaN(this.maxScrollRate)?Number(30):Number(this.maxScrollRate);
            ...
            {
               this.addMouseEventHistory(param1.stageX,param1.stageY,GetTimerUtil.getTimer());
               if(this.dragFunction != null)
               {
                  this.dragFunction(_loc12_,_loc13_);
               }
               param1.updateAfterEvent();
            ...
            else
            {
               ...
               this.mostRecentDragTime = GetTimerUtil.getTimer();
      ...
      private function dragTimerHandler(param1:TimerEvent) : void
      {
         if(this.dragScrollPending)
         {
            ...
            if(this.dragFunction != null)
            {
               this.dragFunction(this.mostRecentDragDeltaX,this.mostRecentDragDeltaY);
            }
            param1.updateAfterEvent();
            this.dragScrollPending = false;
         }
      ...
      private function sbRoot_mouseUpHandler(param1:Event) : void
      {
         ...
         var _loc2_:Number = GetTimerUtil.getTimer();
         var _loc3_:Number = 0;
         ...
         while(_loc7_ != _loc6_)
         {
            ...
            _loc3_ = _loc3_ + (this.mouseEventTimeHistory[_loc11_] - this.mouseEventTimeHistory[_loc7_]);
            _loc7_ = _loc11_;
         }
         _loc3_ = _loc3_ / (_loc4_ - 1);
         var _loc8_:Number = MIN_START_VELOCITY_IPS * Capabilities.screenDPI / 1000;
         ...
         if(this.throwFunction != null)
         {
            this.throwFunction(_loc9_.x,_loc9_.y);
         }
      }
      ...
      private function calculateThrowVelocity() : Point
      {
         ...
         while(_loc7_ != _loc3_)
         {
            ...
            if(_loc13_ != 0)
            {
               _loc4_ = _loc4_ + _loc14_ / _loc13_ * VELOCITY_WEIGHTS[_loc8_];
               _loc5_ = _loc5_ + _loc15_ / _loc13_ * VELOCITY_WEIGHTS[_loc8_];
               _loc6_ = _loc6_ + VELOCITY_WEIGHTS[_loc8_];
            }
            _loc7_ = _loc12_;
            _loc8_++;
         }
         ...
         var _loc9_:Number = MAX_THROW_VELOCITY_IPS * Capabilities.screenDPI / 1000;
      ...
      private function calculateFinalDragVelocity(param1:Number) : Point
      {
         ...
         var _loc4_:int = (this.mouseEventLength - 1) % EVENT_HISTORY_LENGTH;
         ...
         while(param1 > 0 && _loc5_ != _loc3_)
         {
            ...
            if(_loc9_ < 0)
            {
               _loc9_ = _loc9_ + EVENT_HISTORY_LENGTH;
            }
         ...
         if(this.isIOS)
         {
            _loc14_ = MAX_THROW_VELOCITY_IPS * Capabilities.screenDPI / 1000;
      ...
      private function dispatchBubblingEventOnMouseDownedDisplayObject(param1:Event) : Boolean
      {
         ...
         if(this.mouseDownedDisplayObject)
         {
            ...
            {
               if(this.target && !this.target.contains(this.mouseDownedDisplayObject))
               {
                  _loc2_ = _loc2_ && this.target.dispatchEvent(param1);
               }
            }
         }
         else
      ...
      mx_internal function endTouchScroll() : void
      {
         ...
         if(this.isScrolling)
         {
            ...
            _loc1_.relatedObject = this.target;
