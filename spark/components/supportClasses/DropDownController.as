package spark.components.supportClasses
{
   ...
   public class DropDownController extends EventDispatcher
   {
      ...
      public function set openButton(param1:spark.components.supportClasses.ButtonBase) : void
      {
         ...
         if(this._openButton)
         {
            this._openButton.disableMinimumDownStateTime = true;
         }
      ...
      private function addCloseTriggers() : void
      {
         if(this.systemManager)
         {
            ...
            this.addCloseOnResizeTrigger();
            if(this.openButton && this.openButton.systemManager)
            {
      ...
      private function isTargetOverDropDownOrOpenButton(param1:DisplayObject) : Boolean
      {
         ...
         if(param1)
         {
            if(this.openButton && this.openButton.contains(param1))
            {
               return true;
            }
            if(this.hitAreaAdditions != null)
            {
               ...
               {
                  if(this.hitAreaAdditions[_loc2_] == param1 || this.hitAreaAdditions[_loc2_] is DisplayObjectContainer && DisplayObjectContainer(this.hitAreaAdditions[_loc2_]).contains(param1 as DisplayObject))
                  {
                     return true;
                  }
                  _loc2_++;
      ...
      private function openDropDownHelper(param1:Boolean = false) : void
      {
         if(!this.isOpen)
         {
            ...
            if(this.openButton)
            {
               this.openButton.keepDown(true,!param1);
            }
      ...
      mx_internal function systemManager_mouseDownHandler(param1:Event) : void
      {
         ...
         if(!this.dropDown || this.dropDown && (param1.target == this.dropDown || this.dropDown is DisplayObjectContainer && !DisplayObjectContainer(this.dropDown).contains(DisplayObject(param1.target))))
         {
            ...
            {
               _loc3_ = (_loc2_ as IUIComponent).document;
               if("hostComponent" in _loc3_ && _loc3_.hostComponent is DropDownList)
               {
                  return;
               }
            }
            if(this.hitAreaAdditions != null)
            {
               _loc4_ = this.hitAreaAdditions.length;
               _loc5_ = 0;
               while(_loc5_ < _loc4_)
               {
                  if(this.hitAreaAdditions[_loc5_] == _loc2_ || this.hitAreaAdditions[_loc5_] is DisplayObjectContainer && DisplayObjectContainer(this.hitAreaAdditions[_loc5_]).contains(_loc2_))
                  {
                     return;
                  }
                  _loc5_++;
            ...
            if(this.dropDown is IUIComponent)
            {
               if((this.dropDown as IUIComponent).owns(_loc2_))
               {
                  return;
               }
            }
      ...
      mx_internal function systemManager_mouseMoveHandler(param1:Event) : void
      {
         ...
         if(param1 is MouseEvent && MouseEvent(param1).buttonDown || param1 is SandboxMouseEvent && SandboxMouseEvent(param1).buttonDown)
         {
      ...
      {
         if(this.dropDown && !(DisplayObjectContainer(this.dropDown).contains(DisplayObject(param1.target)) && param1.isDefaultPrevented()))
         {
