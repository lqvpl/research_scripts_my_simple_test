package spark.effects
{
   ...
   public class ThrowEffect extends Animate
   {
      ...
      {
         var _loc3_:int = param1 == 0?0:int(Math.log(0.01 / Math.abs(param1)) / Math.log(this.decelerationFactor));
         var _loc4_:int = param2 == 0?0:int(Math.log(0.01 / Math.abs(param2)) / Math.log(this.decelerationFactor));
      ...
      mx_internal function setup() : Boolean
      {
         ...
         this.easer = _loc1_;
         var _loc2_:int = this.calculateThrowEffectTime(this.startingVelocityX,this.startingVelocityY);
         var _loc3_:Vector.<MotionPath> = new Vector.<MotionPath>();
         this.isSnapping = false;
         var _loc4_:Number = 0;
         var _loc5_:Number = 0;
         this.horizontalMP = null;
         if(this.propertyNameX)
         {
            this.horizontalMP = this.createThrowMotionPath(this.propertyNameX,this.startingVelocityX,this.startingPositionX,this.minPositionX,this.maxPositionX,_loc2_);
         ...
         if(this.propertyNameY)
         {
            this.verticalMP = this.createThrowMotionPath(this.propertyNameY,this.startingVelocityY,this.startingPositionY,this.minPositionY,this.maxPositionY,_loc2_);
            if(this.verticalMP)
            {
         ...
         if(_loc3_.length != 0)
         {
            ...
            this.finalPosition = new Point(_loc5_,_loc7_);
            return true;
         }
         return false;
      }
      ...
      mx_internal function getCurrentVelocity() : Point
      {
         ...
         var _loc3_:Number = !!this.horizontalMP?Number(this.getMotionPathCurrentVelocity(this.horizontalMP,_loc1_,_loc2_)):Number(0);
         var _loc4_:Number = !!this.verticalMP?Number(this.getMotionPathCurrentVelocity(this.verticalMP,_loc1_,_loc2_)):Number(0);
      ...
      private function createThrowMotionPath(param1:String, param2:Number, param3:Number, param4:Number, param5:Number, param6:Number) : SimpleMotionPath
      {
         ...
         if(param2 == 0)
         {
            ...
            {
               param3 = param3 < param4?Number(param4):Number(param5);
               if(this.finalPositionFilterFunction != null)
               {
                  param3 = this.finalPositionFilterFunction(param3,param1);
               }
            ...
            else
            {
               ...
               if(this.finalPositionFilterFunction != null)
               {
                  _loc10_ = this.finalPositionFilterFunction(param3,param1);
               }
               if(_loc10_ == param3)
               {
                  return null;
         ...
         while(param2 != 0)
         {
            ...
            {
               _loc11_ = param3 < param4?Number(param4):Number(param5);
               if(this.finalPositionFilterFunction != null)
               {
                  _loc11_ = this.finalPositionFilterFunction(_loc11_,param1);
               }
            ...
            else
            {
               ...
               _loc15_ = Math.round(param3 - param2 / THROW_CURVE_EXPONENT * _loc13_);
               if(_loc15_ < param4 || _loc15_ > param5)
               {
                  _loc16_ = _loc15_ < param4?Number(param4):Number(param5);
                  ...
                  _loc19_ = 3 * Math.pow(_loc18_,2) - 6 * _loc18_ + 3;
                  param2 = -_loc19_ * (_loc15_ - param3) / _loc13_;
               }
               else
               {
                  if(this.finalPositionFilterFunction != null)
                  {
                     _loc15_ = this.finalPositionFilterFunction(_loc15_,param1);
                  }
                  _loc20_ = 0.62;
