package spark.effects.supportClasses
{
   ...
   public class AnimateInstance extends EffectInstance implements IAnimationTarget
   {
      ...
      {
         return !!this.animation?Number(this.animation.playheadTime):Number(this._seekTime);
      }
         ...
         while(_loc4_ < this.motionPaths.length)
         {
            ...
            if(!(!_loc6_ || _loc6_.length == 0))
            {
               if(!this.isValidValue(_loc6_[0].value))
               {
                  ...
                  else if((playReversed || Effect(effect).transitionInterruption) && propertyChanges && propertyChanges.start[_loc5_.property] !== undefined)
                  {
      ...
      private function addDisappearingTarget() : void
      {
         ...
         if(propertyChanges)
         {
            if("parent" in target && !target.parent)
            {
               ...
               {
                  _loc4_ = !playReversed?propertyChanges.start["index"]:propertyChanges.end["index"];
      ...
      private function removeDisappearingTarget() : void
      {
         ...
         var _loc2_:* = undefined;
         var _loc3_:* = undefined;
         if(this.needsRemoval && propertyChanges)
         {
            if("parent" in target && target.parent)
            {
               _loc1_ = propertyChanges.start["parent"];
